export interface CountryConfig {
  ...
  /** Local-currency equivalent of KES 5,000 — triggers low-balance increase prompts */
...
export type ResidenceCountryCode = CountryCode | 'ABROAD'
 ...
 * Automatically cleans leading zeros, removes non-digits, and detects existing international dialing codes.
 */
export function splitPhonePrefix(
  ...
  // 1. If starts with '+', search matching country with the longest matching dial code
  if (raw.startsWith('+')) {
    const allMatches = [
    ...
    if (m) {
      const dial = m[1]
      const digits = m[2].replace(/\D/g, '').replace(/^0+/, '')
      return {
        countryCode: fallbackCountryCode,
        dial,
        digits,
  ...
  // 2. If raw doesn't start with '+', it's a local/national number
  let digits = raw.replace(/\D/g, '')
  const dialDigits = defaultDial.replace(/\D/g, '')

  // If already prefixed with dial digits without the plus sign, strip them
